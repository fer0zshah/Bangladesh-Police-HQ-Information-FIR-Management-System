<x-admin-layout pageTitle="{{ $station->name }} Complaints">
    <div class="mx-auto max-w-[1440px] space-y-6">
        <section class="relative overflow-hidden rounded-xl border border-hq-700 bg-gradient-to-br from-hq-800 via-hq-800 to-hq-700/70 p-5 shadow-xl shadow-black/10 sm:p-6">
            ...
            <div class="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div class="max-w-3xl">
                    ...
                    <h2 class="text-2xl font-bold tracking-tight text-white">{{ $station->name }} complaint workload</h2>
                    <p class="mt-2 text-sm leading-6 text-gray-400">Choose a thana under this command to open its complaint dictionary.</p>
                </div>
                <a href="{{ route('admin.complaints.index') }}" class="inline-flex items-center justify-center rounded-lg border border-hq-600 bg-transparent px-4 py-2.5 text-sm font-bold text-gray-300 transition hover:border-gold-500/50 hover:bg-gold-500/10 hover:text-white">
                    Back to complaint browser
                </a>
            </div>
        </section>
        ...
        <section class="grid grid-cols-2 gap-4 xl:grid-cols-4">
            @foreach ([
                ...
                ['label' => 'Complaints', 'value' => $hqSummary['complaints'], 'note' => 'Across all thanas', 'color' => 'rose'],
                ['label' => 'Pending', 'value' => $hqSummary['pending'], 'note' => 'Awaiting review', 'color' => 'amber'],
                ['label' => 'Escalated', 'value' => $hqSummary['escalated'], 'note' => 'Moved toward FIR', 'color' => 'emerald'],
            ] as $card)
        ...
        <section class="overflow-hidden rounded-xl border border-hq-700 bg-hq-800 shadow-xl shadow-black/5">
            <div class="flex flex-col gap-3 border-b border-hq-700 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="text-base font-semibold text-white">Thana complaint dictionary selector</h3>
                    <p class="mt-1 text-xs text-gray-500">Click a thana to see only complaints submitted to that station.</p>
                </div>
                    ...
                    <table class="w-full min-w-[900px] text-left whitespace-nowrap">
                        <thead>
                            <tr class="border-b border-hq-700 bg-hq-900/60 text-xs font-bold uppercase tracking-widest text-gray-400">
                                ...
                                <th class="px-4 py-4 text-center">Complaints</th>
                                <th class="px-4 py-4 text-center">Pending</th>
                                <th class="px-4 py-4 text-center">Under Review</th>
                                <th class="px-4 py-4 text-center">Escalated</th>
                                ...
                                <tr class="group text-gray-400 transition-colors hover:bg-hq-700/20 hover:text-gray-200">
                                    <td class="px-5 py-4">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                <a href="{{ route('admin.complaints.station', $thana) }}" class="font-semibold text-gray-200 transition hover:text-gold-500">{{ $thana->name }}</a>
                                                <p class="mt-0.5 text-xs text-gray-500">Registry #{{ $thana->station_id }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">{{ $thana->district ?: 'N/A' }}</td>
                                    <td class="px-4 py-4 text-center font-semibold text-gray-300">{{ number_format($thana->complaints_count) }}</td>
                                    <td class="px-4 py-4 text-center font-semibold text-amber-400">{{ number_format($thana->pending_complaints_count) }}</td>
                                    <td class="px-4 py-4 text-center font-semibold text-sky-400">{{ number_format($thana->review_complaints_count) }}</td>
                                    <td class="px-4 py-4 text-center font-semibold text-emerald-400">{{ number_format($thana->escalated_complaints_count) }}</td>
                                    <td class="px-5 py-4 text-right">
                                        <a href="{{ route('admin.complaints.station', $thana) }}" class="rounded-md border border-gold-500/30 bg-transparent px-3 py-1.5 text-xs font-semibold text-gold-400 transition hover:border-gold-500 hover:bg-gold-500/10">View complaints</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
